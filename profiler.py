...
class Profiler:
	...
	def line_trace_function(self, frame, event, arg, s_time):
	...
	def timing_trace_function(self, frame, event, arg, s_time):
		...
		if event == "call":
			if len(self.function_stack) != 0:
				self.function_stack[-1][3] = time.time()
			...
			if self.timing_isolation:
				total_time = (curr_time - prev[1]) * 1000 - prev[2]
			else:
			...
			if len(self.function_stack) != 0:
				self.function_stack[-1][2] += (time.time() - self.function_stack[-1][3]) * 1000
